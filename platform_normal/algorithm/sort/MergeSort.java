...
public class MergeSort {
	...
	/**
	 * 归并排序（Merge Sort）与快速排序思想类似：O(nlogn)
	 * 
	 * 将待排序数据分成两部分，继续将两个子部分进行递归的归并排序；然后将已经有序的两个子部分进行合并，最终完成排序。
	 * 
	 * 但是归并排序除了递归调用间接使用了辅助空间栈
	 * 
	 * 归并排序是一种稳定的排序算法，快速排序则不然。
     * 
     * 一个排序算法是否稳定，影响很小，但是对于结构体数组，稳定排序就十分重要。例如对于student结构体按照关键字score进行非降序排序：
	 */
